import * as reduxFields from '../actions';
import store from 'reduxFieldsStore';
...
export const clearAllFields = ( ) => {
  store.getStore().dispatch(reduxFields.clearAllFields( ));
}

export const getAllFields = ( ) => {
  const fields = store.getStore().getState( ).fields;
  return fields;
}
...
const reduxField = ( field, key, value, propAux ) => {
  ...
  let fields = typeof propAux === 'object'
              ? propAux
              : store.getStore().getState( ).fields;
  ...
  if( key || (typeof propAux === 'string' && propAux === 'remove') ) {
    store.getStore().dispatch(reduxFields.setFields( fields ));
  }
